...
void CG_AddParticles (void)
{
	...
	float			time, time2;
	vec3_t			org;
//	int				color; // sookee: fix warnings (set not used)
	cparticle_t		*active, *tail;
//	int				type; // sookee: fix warnings (set not used)
	vec3_t			rotate_ang;
	...
	for (p=active_particles ; p ; p=next)
	{
		...
		//color = p->color; // sookee: fix warnings (never used)
		...
		//type = p->type; // sookee: fix warnings (never used)
